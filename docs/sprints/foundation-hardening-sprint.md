...
- **T6.2 — en curso.** F6.1: `TreatmentSyncHandler` migró primero **dentro de `sync/`** (desviación del plan original); corregido en el mismo ciclo (**ADR-0008**) a **co-ubicado con su módulo** (`health/sync/treatment-sync.handler.ts`) — diseño original de este ítem. F6.3: `AnimalEventSyncHandler` migró a **`AnimalHistoryModule`**, bounded context permanente nuevo (**ADR-0009**) — `animal_events` no pertenece a herd/health/repro, es un subdominio genérico transversal (ya usado por health/repro/land vía `common/events.ts`; a futuro también genética/IoT/IA). Pendientes: `animals`→herd, `pregnancies`→repro, y `weighings`/`vaccinations`/`breeding_events`/`calvings`/`calving_offspring` (resto de la oleada 2, ver análisis F6 §5).
