...
- **T6.2 — oleada 2 completa (7 de 9 tablas), `animals`/`pregnancies` pendientes.** F6.1: `TreatmentSyncHandler` migró primero **dentro de `sync/`** (desviación del plan original); corregido en el mismo ciclo (**ADR-0008**) a **co-ubicado con su módulo** (`health/sync/treatment-sync.handler.ts`). F6.3: `AnimalEventSyncHandler` migró a **`AnimalHistoryModule`**, bounded context permanente nuevo (**ADR-0009**) — subdominio genérico transversal, no propiedad de herd/health/repro. F6.3-B: `VaccinationSyncHandler` (health), `WeighingSyncHandler` (herd), `BreedingEventSyncHandler`/`CalvingSyncHandler`/`CalvingOffspringSyncHandler` (repro) — mismos INSERT sin lógica de negocio, defaults preservados exactamente. **`applyEvent()` y `EVENT_TABLES` se eliminaron por completo** de `sync.service.ts` (ya no les quedaba ninguna tabla). Pendientes: `animals`→herd, `pregnancies`→repro — fase posterior, mayor complejidad (LWW, conflictos, Server Authority).
