...
| 2026-08-15 | `largestServiceLines` | 1094 → **978** | Se partió `alerts.service.ts` (1094 → 477) extrayendo `AlertRulesService`, el motor que decide qué alertas deberían existir. El mayor pasa a ser `repro.service.ts` (978). |
| 2026-08-15 | `MAX_SERVICE_LINES` (gate) | 1150 → **1000** | Ahora sí corresponde bajar el trinquete: con el peor caso en 978, un techo de 1150 dejaba 172 líneas de margen y ya no frenaba nada — que es lo contrario de lo que un trinquete hace. A 1000 quedan 22 líneas sobre `repro.service.ts`, la misma holgura ajustada con la que se fijó 1150 sobre 1141. **`repro.service.ts` es el próximo a partir.** Nota de método: el gate solo mira archivos `*.service.ts`, así que una extracción a `*.engine.ts` o similar lo esquivaría sin querer; por eso el motor de alertas se llama `alert-rules.service.ts` y no `alert-rules.engine.ts`. |
| 2026-08-15 | `largestServiceLines` | 978 → **940** | Se partió `repro.service.ts` (978 → 795) extrayendo `ReproStatusService`. El mayor vuelve a ser `herd.service.ts` (940). **El techo se dejó en 1000, no se bajó a ~950.** Acá la campaña de "partir el servicio más grande" llegó a su límite útil: a `herd.service.ts` le queda el CRUD del animal —listar, ver, crear, editar, ficha 360—, que es un solo caso de uso. Las tres particiones anteriores cortaron por costuras que ya existían en el archivo; la próxima tendría que cortar contra la veta, y un techo que obliga a eso deja de proteger el diseño para empezar a deformarlo. Si el tamaño vuelve a molestar, el candidato honesto es `db/seed.ts` (1309), que el gate no mira porque es data. |
