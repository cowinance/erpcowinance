...
### Resumen ejecutivo
Cowinance es una **plataforma ERP bovina** (carne/leche/mixta), offline-first y multi-tenant. El **alcance ganadero de Fase 1 está construido y verificado** (Hato, Sanidad, Reproducción, Producción/manga, Mapa, Reportes, Alertas, Fotos, Planes sanitarios), sobre 3 apps (`api` NestJS, `web` Next.js, `mobile` Expo) + 4 packages (`db`, `sync-core`, `domain`, `design-tokens`). El **Foundation Hardening Sprint (F0-F9) está COMPLETO** — dominio puro con VOs y servicios, Server Authority en sync, sync en handlers por bounded context, Event Bus + Outbox, dashboard desacoplado, 9 ADRs (0001-0009), y `audit:arch`. El proyecto está en la **Fase Producto**: ADR-0010 (P1.1), ADR-0011 (P1.2), ADR-0012 (P1.3), ADR-0013 (P1.4.1). **P1.1 (registro self-service), P1.2 (email/verificación/reset), P1.3 (onboarding y experiencia inicial web+móvil) y P1.4.1 (fuente única de tokens) están COMPLETOS, validados y pusheados. P1.4.2 (especificación del Design System, ADR-0014) y P1.4.3 (aplicación de la escala tipográfica y de spacing, web + móvil) están COMPLETOS, pusheados y cerrados.** P1.3 llevó el onboarding SaaS a la UI **sobre el diseño existente** (no fue un rediseño); P1.4 abrió los **fundamentos de sistema de diseño**: **auditoría** (aprobada), **P1.4.1** cableó el **pipeline de tokens**, **P1.4.2** entregó la **especificación completa** (ADR-0014, 7 decisiones), y **P1.4.3 la aplicó** (roles `type-*` + `T.space`, web+móvil, behavior-preserving; números mágicos eliminados salvo exclusiones documentadas; ver §0sexies). **P1.4.4 (primitivos + densidad) CERRADO** (ADR-0015 + [`docs/design-system/P1.4.4-closure.md`](../design-system/P1.4.4-closure.md)): densidad runtime por tamaño (`--density-control-h-{sm,md,lg}` = 32/36/40) y primitivos web **Button/Input/Select/Field** **aplicados en toda la web** (auth, alta de animal, Sanidad, Reproducción, Reportes, HealthPlans, FarmMap, WeighingForm…), behavior-preserving; sin control/botón estándar fuera de primitivos salvo excepciones documentadas (búsqueda con icono, icon-only, danger, secondary-sunken, CTA-Link). Con esto, **P1.4 (Fundamentos de experiencia + Design System) está COMPLETO y CERRADO**. Según [product-roadmap-2026](../product/product-roadmap-2026.md), el siguiente tramo comprometido del roadmap es **P2 (importación/migración del hato)**; su inicio requiere autorización explícita. Ver §9.
...
## 0sexies. Fase Producto — P1.4.3 COMPLETADO ✅ (aplicación de la escala tipográfica y de spacing, web + móvil)

**Objetivo P1.4.3 (cumplido):** aplicar la escala **tipográfica** (roles `type-*`) y de **spacing** de la spec/ADR-0014 en web y móvil, **estrictamente behavior-preserving** (mismos valores visuales; solo relocalizados a tokens de la fuente única). Eliminar los números mágicos (`text-[Npx]`, `fontSize: N`, spacing numérico). **NO** se implementó dark, iconografía, primitivos, responsive ni **aplicación** de densidad — eso es P1.4.4+. **Pusheado y cerrado.** Cierre documental en curso (este handoff + ADR-0014 nota de implementación).

**Método rector:** roles migrados **1:1 al tamaño actual** (sin line-height ideal de la spec §2 — Tailwind v4 `@theme --text-<rol>` compila a solo `font-size`; verificado en el piloto). Peso/tracking/color/line-height siguen como ejes separados. Divergencias web/móvil **preservadas, no unificadas** (KPI móvil 26 vs display web 30). Cada lote: gates + revisión visual dirigida claro/oscuro (web) / Expo (móvil); commits chicos por área; pausa de revisión del owner por lote.

**Commits (en `origin/main`), en orden:**
- **Web tipografía:** `8ead5a3` (infra: tokens.ts roles/aliases/spacing/densidad + generador) · `fbd5a40` (piloto KpiCard) · `99e2183` (compartidos + auth) · `191336b` (Hato) · `a3363ea` (Sanidad/Reproducción) · `04e0672` (datos densos: dashboard/alertas/sync/reportes/FarmMap/WeightChart) · `d178bc5` (Lotes).
- **Móvil tipografía:** `c08a9b8` (adaptador `T.type`/`T.compat`) · `c8caa35` (piloto Kpi) · `1f97339` (compartidos + shell) · `ad08e50` (Home+Animales) · `0b68f77` (Captura+AnimalPicker) · `a8b263c` (Sync+Mapa+detalle).
- **Móvil spacing:** `f5605a6` (adaptador `T.space`) · `5331020` (piloto ui.tsx) · `177439b` (`space['2.5']=10`) · `57262dd` (compartidos+shell) · `dad79b2`+`b129657`+`60c8553` (funcionales por área).

**Estado del sistema (medido al cierre):**
- **Fuente (`tokens.ts`):** `fontSize` (rampa primitiva), `typeRole` (roles estables), `typeCompat` (aliases deuda), `space` (escala spacing, incluye `'2.5':10`), `density` (contrato, un modo).
- **Web:** todos los `text-[Npx]` migrados a roles `text-*` salvo exclusiones. Spacing web **no migrado a propósito** (ya usa la escala de Tailwind).
- **Móvil:** todos los `fontSize`/spacing numéricos migrados a `T.type.*`/`T.space['*']` salvo exclusiones.

**Exclusiones documentadas (deuda de convergencia futura, NO ocultadas):**
- **Web:** `text-xl` (named, line-height propio); `fontSize` SVG (FarmMap/WeightChart); `text-[14px]` de encabezados (reportes/alertas); base raíz `<body>` de `layout.tsx`; **manga web** (bespoke) + sus hex `#4ade80/#f87171` (color, fuera de alcance); página placeholder de módulos no implementados.
- **Móvil:** caravanas **mono** (12/15/16/32 px, identificadores de dominio); **contenido 14 px** (filas/valores/cuenta); **inputs 15 px** históricos; **logo** login; **manga móvil** (bespoke). Spacing: `7 px` óptico (Sync) + `14 px` (un gap + dos paddingHorizontal); **sin `space['3.5']`**.

**Aliases `typeCompat` (congelado):** 7 con consumidores (compat-9/10/16/22/24/26/28), **4 sin consumidores** (compat-18/32/48/64 → deuda; sus valores terminaron en manga o exclusión literal). No se podaron en el cierre (sería runtime; candidato a poda mecánica futura).

**Densidad:** contrato definido (`density.standard`), **NO aplicado** ni emitido. Alturas/touch (h-9 web, 44 móvil, …) preservadas literales; **prohibido introducir alturas interactivas hardcodeadas nuevas** (P1.4.4 las cablea).

**Gates finales P1.4.3 — todos verdes:** `tokens:check` ✓ · typecheck design-tokens/web/móvil/api/domain/sync-core ✓ · `next build` + `nest build` limpios ✓ · `audit:arch` (114 tests, 0 ciclos) ✓ · **Playwright 5/5** ✓ · **Expo/Metro** bundea sin errores ✓ · **sim 2000/2000 (100%)** ✓ · revisión visual dirigida acumulada (web claro/oscuro en todas las áreas; móvil en todas las pantallas): **sin diferencias**. Backend E2E: P1.4.3 no tocó código backend → sin cambios.

**Siguiente (P1.4.4):** primitivos por familia (incl. cerrar la divergencia `Button` web) **cableando la capa de densidad** (`--control-h`/`row-h`/… — la primera aplicación de densidad); luego navegación responsive + honestidad de módulos (P1.4.5), primitivos móvil + mapa de iconos (P1.4.6), dark web auditado (P1.4.7). Al construir primitivos: normalizar los line-height/peso/tracking (fase visible, con aprobación), converger las exclusiones (caravanas → rol de dato, 14/15 px, base raíz) y podar los 4 aliases sin uso.

---

## 0quinquies. Fase Producto — P1.4.2 COMPLETADO ✅ (especificación del Design System, ADR-0014)

**Objetivo P1.4.2 (cumplido):** la **especificación técnica completa** del Design System como **contrato** del frontend futuro (web + móvil), **fase exclusivamente conceptual** — sin tocar pantallas, componentes, CSS ni tokens. Aprobada por el owner sin cambios. **ADR-0014** registra las decisiones de arquitectura. **COMPLETADO, commiteado y pusheado a `origin/main` (commit `66d700a`); ADR-0014 aprobado. Cerrado oficialmente.**

**Entregables (docs, sin código):**
- **`docs/design-system/P1.4.2-design-system-spec.md`** (nuevo) — spec de 14 secciones: filosofía, escala tipográfica (11 roles semánticos `type-*`), spacing (grid 4px + sub-unidad 2px), densidad ERP (contrato cómodo/estándar/compacto), colores (reglas de uso, sin cambiar paleta), dark mode (análisis + recomendación), iconografía (contrato + mapa semántico lucide↔Ionicons), inventario de componentes (7 familias con prioridad/deps/consumidores), motion, accesibilidad AA, responsive, riesgos técnicos, roadmap P1.4.3–P1.4.6, y qué va/no va en ADR-0014.
- **`docs/adr/0014-design-system-specification.md`** (nuevo) — 7 decisiones de arquitectura (A–G): (A) API tipográfica = roles semánticos, no tamaños sueltos; (B) grid 4px + sub-unidad 2px; (C) **densidad como capa multiplicadora en runtime, introducida desde P1.4.3** (evita retrabajo); (D) **dark web controlado por `data-theme` y auditado en P1.4.7; móvil claro-primero, dark diferido**; (E) iconos = contrato de significado, dos librerías permitidas (sin vendor único, YAGNI); (F) accesibilidad **WCAG 2.1 AA** como piso; (G) breakpoints + resolución del **gap de nav web < lg** (sidebar → top bar + drawer).
- Índice de ADR (`docs/adr/README.md`), `README.md` (§Design System) actualizados.

**Hallazgos de auditoría clave (medidos en el código):** 18 tamaños `text-[Npx]` distintos en web (workhorse 13px, 103 usos); divergencias accidentales web/móvil (KPI 30 vs 26, Card 20 vs 16, web sin primitivo `Button`); **dark mode por accidente** (web sigue `@media prefers-color-scheme` con contrastes sin auditar; móvil solo claro); dos librerías de iconos sin contrato; **gap de navegación** bajo `lg` (sidebar desaparece sin reemplazo).

**Fuera de alcance (evolución natural, no bloquea):** tablas empresariales avanzadas, dashboards analíticos, formularios complejos, i18n visual (RTL) y componentes para IA — se amplían cuando sus módulos entren en desarrollo.

**Gates P1.4.2 — verdes:** cambio **docs-only** (no toca código); `audit:arch` verde (`tokens:check` ✓, typechecks incl. mobile ✓, 114 tests ✓, 0 ciclos). `next build`/`nest build`/Playwright no aplican (sin cambios de runtime).

**Siguiente:** **P1.4.3 ya está cerrado** (aplicó la escala; ver §0sexies). Sigue **P1.4.4** (primitivos + aplicación de densidad).
...
### Último commit
**Último commit remoto (`origin/main`): `66d700a`** — `docs(design-system): P1.4.2 — especificación del Design System + ADR-0014` (cierre P1.4.2, ver §0quinquies). Antes: cadena P1.4.1 `e55e233` → `0dbb852` → `fc7addf` → `d770448` (§0quater); P1.3 (14 commits, §0ter). **Local = remoto (0/0); sin commits pendientes de push** salvo, si aplica, el propio commit documental de este cierre de estado.
...
**Foundation Hardening Sprint COMPLETO (F0-F9)**. **Fase Producto en curso** — **P1.1 (ADR-0010), P1.2 (ADR-0011), P1.3 (ADR-0012), P1.4.1 (ADR-0013), P1.4.2 (ADR-0014) y P1.4.3 (aplicación de la escala tipográfica y de spacing, web + móvil) COMPLETOS, validados y pusheados** (ver §0…§0sexies). `npm run audit:arch` verde (114 tests, 0 ciclos; **Gate 0 `tokens:check`**); Playwright 5/5; sim 2000/2000. **Siguiente: P1.4.4 — primitivos por familia + primera aplicación de densidad (§9).**
...
1. **P1.4.4 — Primitivos por familia + primera aplicación de densidad** ← **siguiente paso** (ver §0sexies / §9). Sobre la escala tipográfica y de spacing **ya aplicada en P1.4.3** (web + móvil, behavior-preserving), construir los primitivos (Button web incl. cerrar la divergencia, Input, DataTable, Modal/Drawer, Toast, Icon…) **cableando la capa de densidad** (`--control-h`/`row-h`/…, ADR-0014 dec. C — primera aplicación real). Regla dura vigente: **prohibido hornear alturas/paddings verticales interactivos nuevos**. Aprovechar para **normalizar** line-height/peso/tracking (fase visible), **converger** exclusiones documentadas (caravanas mono, contenido 14px, inputs 15px, base raíz) y **podar** los 4 aliases `typeCompat` sin consumidores. Behavior-preserving en lo ya migrado; Playwright + `tokens:check` + Expo/Metro + sim como gates. (P1.4.1/P1.4.2/P1.4.3 cerrados y pusheados; ver §0sexies / §0quinquies / §0quater.)
2. **Candidatos posteriores** (ver §9): P1.4.5 (nav responsive + honestidad de módulos, resolver gap < lg), P1.4.6 (primitivos móvil + mapa de iconos), P1.4.7 (dark web auditado); invitaciones/roles; CRUD de lotes y potreros; rate limiting; cookies `httpOnly`; deep-linking móvil; stores móviles namespaced.
...
> Foundation Hardening (F0-F9) **cerrado**; P1.1/P1.2/P1.3/P1.4.1/P1.4.2 (Fase Producto) **cerrados y pusheados** — no hay pasos pendientes de esos tramos.
...
> **P1.4.4 (primitivos + densidad) CERRADO.** Primitivos **Button/Input/Select/Field** cableados a la densidad por tamaño y **aplicados en toda la web**, behavior-preserving; deuda diferida documentada (icon-only, danger, secondary-sunken, búsqueda con icono, CTA-Link). Con esto **P1.4 completo**. Cierre técnico en `docs/design-system/P1.4.4-closure.md`. Según `docs/product/product-roadmap-2026.md`, el siguiente tramo comprometido del roadmap es **P2 (importación/migración del hato)**; su inicio requiere autorización explícita.

**P1.4 (Fundamentos de experiencia y sistema de diseño) está en curso.** Cerrados y pusheados: **auditoría** (aprobada), **P1.4.1 — fuente única de tokens** (ADR-0013), **P1.4.2 — especificación del Design System** (ADR-0014 aprobado, commit `66d700a`, ver §0quinquies) y **P1.4.3 — aplicación de la escala tipográfica y de spacing** (web + móvil, behavior-preserving; nota de implementación en ADR-0014; ver §0sexies). La spec (`docs/design-system/P1.4.2-design-system-spec.md`) es el **contrato** del frontend futuro; ADR-0014 fija las 7 decisiones de arquitectura (roles tipográficos, grid 4px+2px, **densidad como capa runtime — definida en P1.4.3, aplicada desde P1.4.4**, **dark web controlado/auditado + móvil claro-primero**, iconos con dos librerías y contrato de significado, **AA**, breakpoints + gap de nav < lg). El siguiente tramo **cablea** la densidad y construye primitivos, behavior-preserving:

**P1.4.4 — Primitivos por familia + primera aplicación de densidad.** Sobre la escala ya aplicada en P1.4.3, construir los primitivos (Button web incl. cerrar la divergencia, Input, DataTable, Modal/Drawer, Toast, Icon…) **cableando la capa de densidad** (`--control-h`/`row-h`/…) — primera aplicación real del contrato de densidad (ADR-0014 dec. C). **Regla dura vigente:** prohibido hornear alturas/paddings verticales interactivos nuevos — deben referenciar la capa de densidad, para que agregar cómodo/compacto en P1.4.7 sea cambiar tokens, no reescribir componentes. Aprovechar para **normalizar** line-height/peso/tracking (fase visible, con aprobación), **converger** las exclusiones documentadas en P1.4.3 (caravanas mono → rol de dato, contenido 14px, inputs 15px, base raíz) y **podar** los 4 aliases `typeCompat` sin consumidores (`compat-18/32/48/64`). Después: navegación responsive + honestidad de módulos (P1.4.5, resolver gap < lg), primitivos móvil + mapa de iconos (P1.4.6), dark web auditado/pulido (P1.4.7) — todo behavior-preserving, Playwright + `tokens:check` + Expo/Metro + sim como gates.
...
- **P1.4/ADR-0013/ADR-0014:** P1.4.1 dejó la **fuente única de tokens** (color/radio/sombra/fuente) y **P1.4.2 la especificación completa** del resto del sistema (escala tipográfica, spacing, densidad, dark, iconografía, primitivos, responsive → ADR-0014). **Falta implementarla** (P1.4.3+): la spec está aprobada, ninguna pantalla tocada aún. Riesgos vivos registrados en la spec §12 (DR6 densidad horneada = alto; DR3 FOUC de `data-theme` en Next; DR2 generador a dos targets; contraste `text-tertiary` a auditar). DX: cambiar tokens exige `tokens:build` + commitear el CSS generado (sin watcher de dev por ahora, a propósito).
...
8bis. **Design System / tokens (P1.4.1, ADR-0013)** — `packages/design-tokens/src/tokens.ts` (fuente única editable), `packages/design-tokens/scripts/gen-web-css.mjs` (generador), `apps/web/src/app/tokens.generated.css` (artefacto web, no editar), `apps/web/src/app/globals.css` + `apps/mobile/src/theme.ts` (consumidores). Punto de partida de **P1.4.3**.
8ter. **Especificación del Design System (P1.4.2, ADR-0014)** — **`docs/design-system/P1.4.2-design-system-spec.md`** (contrato completo del frontend: roles tipográficos, spacing, densidad, dark, iconografía, inventario, motion, a11y, responsive, riesgos, roadmap) + **`docs/adr/0014-design-system-specification.md`** (7 decisiones de arquitectura). **Leer antes de arrancar P1.4.3.**
...
**Siguiente acción concreta:** abrir **P1.4.4 — primitivos por familia (Button web, Input, DataTable, Modal/Drawer, Toast, Icon…) cableando la capa de densidad** (`--control-h`/`row-h`/… — primera aplicación real de densidad, ADR-0014 dec. C), sobre la escala ya aplicada en P1.4.3. Regla dura vigente: **no hornear alturas interactivas nuevas** (referenciar la capa de densidad). Al construir primitivos, además: **normalizar** line-height/peso/tracking (fase visible, con aprobación), **converger** las exclusiones documentadas (caravanas mono → rol de dato, contenido 14px, inputs 15px, base raíz) y **podar** los 4 aliases `typeCompat` sin consumidores. Behavior-preserving en lo ya migrado; Playwright + `tokens:check` + Expo/Metro + sim como gates. Ver §0sexies / §9.
