...
> **P1.4.4 (primitivos + densidad) EN CURSO.** Hecho y pusheado: ADR-0015 (mecanismo de densidad + eje de tamaño), vars `--density-control-h-{sm,md,lg}` (32/36/40), primitivos **Button** (piloto auth) e **Input/Select/Field** (piloto `NewAnimalForm`), behavior-preserving. Patrón y divergencias en `docs/design-system/primitives.md`. **Siguiente:** autorizar los próximos consumidores por capítulo (auth `lg`, formularios ganaderos, filtros compactos, Button ganadero).
