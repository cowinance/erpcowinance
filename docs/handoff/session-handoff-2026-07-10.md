...
**Foundation Hardening Sprint** en curso. Completadas: **F0, F1, F2, F3, F4, F6, F5, F7 (Dashboard → `DashboardService`, Opción C — `DashboardProjection` diferido; respuesta idéntica byte a byte)**. Siguiente: **F8** (ADRs retrospectivos 0001-0003) → **F9** (métricas). Ver §5.
...
| **F7** | Dashboard → `DashboardService` (8 SQL fuera del controller); `DashboardProjection` diferido (Opción C) | ✅ Completado ← **siguiente: F8** |
