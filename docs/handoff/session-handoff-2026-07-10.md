...
**Foundation Hardening Sprint** en curso. Completadas: **F0, F1, F2, F3, F4, F6, F5 (Event Bus interno + Outbox, fundación — ADR-0005; evento `TreatmentApplied` emitido desde REST, relay poller, suscriptor de logging)**. Siguiente: **F7** (Dashboard → service + costura de proyección). Ver §5.
...
| **F5** | Event Bus interno + Outbox (fundación) — `TreatmentApplied` desde REST, relay poller, at-least-once (ADR-0005) | ✅ Completado ← **siguiente: F7** |
