...
**Siguiente:** revisar ADR-0014 con el owner → si OK, **autorizar push** y abrir **P1.4.3** (aplicar escala tipográfica + spacing + capa de densidad con un solo modo; migrar los `text-[Npx]`/gaps mágicos a roles; **sin cambiar el look**; prohibido hornear alturas fijas en primitivos interactivos).
...
> Foundation Hardening (F0-F9) **cerrado**; P1.1/P1.2/P1.3/P1.4.1/P1.4.2 (Fase Producto) **cerrados** — no hay pasos pendientes de esos tramos (P1.4.2 pendiente solo de push autorizado).
...
> **Único siguiente paso recomendado: revisar ADR-0014 → autorizar push de P1.4.2 → abrir P1.4.3 (aplicar escala, conceptual→implementación behavior-preserving).**

**P1.4 (Fundamentos de experiencia y sistema de diseño) está en curso.** Cerrados: **auditoría** (aprobada), **P1.4.1 — fuente única de tokens** (pusheado, ADR-0013) y **P1.4.2 — especificación del Design System** (commiteado, pendiente de push; ADR-0014, ver §0quinquies). La spec (`docs/design-system/P1.4.2-design-system-spec.md`) es el **contrato** del frontend futuro; ADR-0014 fija las 7 decisiones de arquitectura (roles tipográficos, grid 4px+2px, **densidad como capa runtime desde P1.4.3**, **dark web controlado/auditado + móvil claro-primero**, iconos con dos librerías y contrato de significado, **AA**, breakpoints + gap de nav < lg). El siguiente tramo **aplica** la spec, behavior-preserving:
