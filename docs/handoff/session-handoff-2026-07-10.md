...
**Siguiente:** abrir **P1.4.3** (aplicar escala tipográfica + spacing + capa de densidad con un solo modo; migrar los `text-[Npx]`/gaps mágicos a roles; **sin cambiar el look**; prohibido hornear alturas fijas en primitivos interactivos).
...
**Foundation Hardening Sprint COMPLETO (F0-F9)**. **Fase Producto en curso** — **P1.1 (ADR-0010), P1.2 (ADR-0011), P1.3 (ADR-0012), P1.4.1 (fuente única de tokens, ADR-0013) y P1.4.2 (especificación del Design System, ADR-0014) COMPLETOS, validados y pusheados** (ver §0 / §0bis / §0ter / §0quater / §0quinquies). `npm run audit:arch` verde (114 tests, 0 ciclos, cobertura 72.54% domain+sync-core; **Gate 0 `tokens:check`**); Playwright 5/5. **Siguiente: P1.4.3 — aplicar escala tipográfica + spacing + capa de densidad (1 modo), behavior-preserving, sin cambiar el look** (§9).
...
1. **P1.4.3 — Aplicar escala tipográfica + spacing + capa de densidad (1 modo)** ← **siguiente paso** (ver §9), tras autorizar el push de P1.4.2: extender `tokens.ts` con tipografía/spacing/densidad; regenerar CSS web + adaptador móvil; **migrar los `text-[Npx]`/gaps mágicos a roles `type-*`**, **sin cambiar el look** (los roles mapean a los valores actuales); introducir la **capa de densidad con un solo modo activo** — **prohibido hornear alturas/paddings verticales fijos** en primitivos interactivos (ADR-0014 dec. C). Behavior-preserving; Playwright + `tokens:check` como gates. (P1.4.1 tokens y P1.4.2 spec+ADR-0014 ya cerrados; ver §0quinquies / §0quater.)
2. **Candidatos posteriores** (ver §9): P1.4.4 (primitivos por familia, incl. cerrar divergencia `Button` web), P1.4.5 (nav responsive + honestidad de módulos, resolver gap < lg), P1.4.6 (primitivos móvil + mapa de iconos), P1.4.7 (dark web auditado); invitaciones/roles; CRUD de lotes y potreros; rate limiting; cookies `httpOnly`; deep-linking móvil; stores móviles namespaced.
...
> **Único siguiente paso recomendado: abrir P1.4.3 (aplicar la escala, conceptual→implementación behavior-preserving).**

**P1.4 (Fundamentos de experiencia y sistema de diseño) está en curso.** Cerrados y pusheados: **auditoría** (aprobada), **P1.4.1 — fuente única de tokens** (ADR-0013) y **P1.4.2 — especificación del Design System** (ADR-0014 aprobado, commit `66d700a`, ver §0quinquies). La spec (`docs/design-system/P1.4.2-design-system-spec.md`) es el **contrato** del frontend futuro; ADR-0014 fija las 7 decisiones de arquitectura (roles tipográficos, grid 4px+2px, **densidad como capa runtime desde P1.4.3**, **dark web controlado/auditado + móvil claro-primero**, iconos con dos librerías y contrato de significado, **AA**, breakpoints + gap de nav < lg). El siguiente tramo **aplica** la spec, behavior-preserving:

**P1.4.3 — Aplicar escala tipográfica + spacing + capa de densidad (1 modo).** Extender `tokens.ts` con tipografía (roles `type-*`)/spacing/densidad; regenerar el CSS web + adaptador móvil; **migrar los `text-[Npx]`/gaps mágicos a roles**, **sin cambiar el look** (los roles mapean a los valores actuales); introducir la **capa de densidad con un solo modo activo**. **Regla dura (ADR-0014 dec. C):** prohibido hornear alturas/paddings verticales fijos en primitivos interactivos — deben referenciar la capa de densidad, para que agregar cómodo/compacto en P1.4.7 sea cambiar tokens, no reescribir componentes. Después: primitivos por familia (P1.4.4a/b/c, incl. cerrar la divergencia `Button` web), navegación responsive + honestidad de módulos (P1.4.5, resolver gap < lg), primitivos móvil + mapa de iconos (P1.4.6), dark web auditado/pulido (P1.4.7) — todo behavior-preserving, Playwright + `tokens:check` como gates.
...
**Siguiente acción concreta:** revisar **ADR-0014** con el owner → **autorizar el push** de P1.4.2 → abrir **P1.4.3 — aplicar escala tipográfica + spacing + capa de densidad (1 modo)**, migrando los `text-[Npx]`/gaps mágicos a roles `type-*` **sin cambiar el look**, con la regla dura de **no hornear alturas fijas** en primitivos interactivos (ADR-0014 dec. C). Todo behavior-preserving; Playwright + `tokens:check` como gates. La spec (`docs/design-system/P1.4.2-design-system-spec.md`) es el contrato; ADR-0014 fija las 7 decisiones. Ver §9 / §0quinquies.
