...
/**
 * KPIs del panel principal. Lectura cross-domain (herd/health/repro) en vivo
 * sobre el modelo transaccional.
 *
 * F7: el SQL se movió VERBATIM desde `dashboard.controller` — el controller
 * queda delgado. Este método `kpis()` es la costura de proyección: el día que
 * su interior pase de live-query a un read model alimentado por eventos
 * (CQRS-lite, futuro), el controller no cambia. No se introduce una interfaz
 * `DashboardProjection` todavía — sería abstracción prematura con una sola
 * implementación (ver sprint doc, decisión de alcance de F7).
 */
@Injectable()
