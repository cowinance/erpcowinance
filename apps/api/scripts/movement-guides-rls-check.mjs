/**
 * Guardia RLS de movement_guides (T-1). Verifica bajo rol NO-super que la política estándar
...
await db.exec(`
  CREATE TABLE movement_guides (
    ...
    guide_number varchar(255) NOT NULL, issued_at date NOT NULL DEFAULT now(), animal_count integer,
  ...
  CREATE POLICY tenant_isolation ON movement_guides
    USING (tenant_id = current_setting('app.tenant_id', true)::uuid)
    WITH CHECK (tenant_id = current_setting('app.tenant_id', true)::uuid);
  INSERT INTO movement_guides (tenant_id, company_id, guide_number, animal_count) VALUES ('${A}','${CO}','G-A',10), ('${B}','${CO}','G-B',20);
  ...
  GRANT SELECT, INSERT, UPDATE ON movement_guides TO appuser;
`);
...
console.log('── Guardia RLS de movement_guides (rol no-super) ──');

const aSees = await inCtx(A, () => db.query('SELECT guide_number FROM movement_guides'));
aSees.rows.length === 1 ? ok('tenant A ve solo su guía') : bad('A debería ver 1', JSON.stringify(aSees.rows));
try {
  await inCtx(A, () => db.query(`INSERT INTO movement_guides (tenant_id, company_id, guide_number) VALUES ($1,$2,'colada')`, [B, CO]));
  bad('WITH CHECK debería bloquear insertar a nombre de B');
} catch {
  ok('WITH CHECK bloquea insertar a nombre de otro tenant');
}
const upd = await inCtx(A, () => db.query(`UPDATE movement_guides SET status='hack' WHERE tenant_id=$1 RETURNING id`, [B]));
upd.rows.length === 0 ? ok('A no puede modificar guías de B') : bad('A no debería tocar filas de B');
