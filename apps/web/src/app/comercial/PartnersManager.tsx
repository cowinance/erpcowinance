    ...
    <div className="grid grid-cols-3 gap-4 max-lg:grid-cols-1">
      ...
      <Card className="self-start">
        ...
        <div className="space-y-2">
          ...
          <Input value={taxId} onChange={(e) => setTaxId(e.target.value)} placeholder="CUIT / identificación fiscal" aria-label="Identificación fiscal" />
              ...
              <li key={p.id} className="flex items-center justify-between py-2">
                <div>
                  ...
                  <div className="text-label text-ink-3">
                    {[catLabel(p.supplier_category) && `Prov: ${catLabel(p.supplier_category)}`, segLabel(p.customer_segment) && `Cli: ${segLabel(p.customer_segment)}`].filter(Boolean).join(' · ') || p.email}
                  </div>
                </div>
